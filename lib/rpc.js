...
class Rpc extends events {
	...
	{
		super();
		this.sub = redis.createClient.apply(redis, settings);
		this.pub = redis.createClient.apply(redis, settings);
					...
					try
					{
						...
						else
						{
							result
								...
								.fail(function(e)
								{
									var message = 'error calling method ' + data.method + ' on object ' +  data.object + '\n';
									if (e.toString) message += e.toString() + '\n';
									if (e.stack) message += e.stack + '\n';
									console.error(e.stack);
									this.fail(data.answerId, message);
								}.bind(this))
						}
					}
					catch (e)
